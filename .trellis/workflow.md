...
> **UniHub 扩展技能路由**: 项目额外提供了 `.trellis/spec/guides/agent-workflow.md`（任务分类系统）和 `.trellis/skill-defaults.json`（类别→技能映射）。详见 `.trellis/spec/guides/agent-workflow.md` 的 Trellis 分类速览表和 UniHub 常见任务速查表。
...
Write the results to `.trellis/workspace/alan/learnings/errors.md` (recovered from historical workflow memory). This file serves as the project's institutional memory for recurring bug patterns.
