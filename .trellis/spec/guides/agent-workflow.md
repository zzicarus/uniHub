...
> UniHub 的 AI 代理任务分类标准。定义不同任务类型对应的分类、技能加载和委派策略。
>
> **来源**: 从历史工作流恢复，路径已适配到 `.trellis/`
> **恢复日期**: 2026-05-23
...
2. 查阅本文件确定分类、技能和委派策略
3. 对于复杂任务，先阅读涉模块的 `AGENTS.md` 和相关 `.trellis/spec/` 文件
...
| **委派前必须检查技能映射** | 每次子代理委派前先读 `.trellis/skill-defaults.json`，按 category 和任务类型确定 `load_skills`；视觉任务必须加载 `frontend-ui-ux` |
...
| 步骤 | 动作 | 说明 |
|------|------|------|
| 1 | 判断 category | 使用下方 Trellis 分类速览 |
...
**加载技能**: 见 `.trellis/skill-defaults.json` → `categoryDefaults.ultrabrain`

**前置阅读**:
- `lib/src/core/AGENTS.md` — 基础设施全景
- `lib/src/core/plugin/AGENTS.md` — 插件系统约定
- `.trellis/workflow.md` — 分层依赖顺序
...
**加载技能**: 见 `.trellis/skill-defaults.json` → `categoryDefaults.unspecified-high` + `taskOverrides.unit_test`
...
**加载技能**: 见 `.trellis/skill-defaults.json` → `categoryDefaults.visual-engineering`

> `frontend-ui-ux` 强制造型检查（配色系统、间距、字体选择、动效质量），每个视觉任务必须加载。

**前置阅读**:
- `.trellis/spec/frontend/component-guidelines.md` — Widget 类型选择、Token 使用、M3 颜色映射
- `.trellis/skill-defaults.json` — 技能默认加载规则
...
**加载技能**: 见 `.trellis/skill-defaults.json` → `taskOverrides.unit_test`
...
**加载技能**: 见 `.trellis/skill-defaults.json` → `taskOverrides.widget_test`
...
- 写 PRD（`.trellis/tasks/archive/plans/prd-*.md`）
- 更新 `.trellis/spec/` 规范
- 写 README、CHANGELOG
- **知识同步 (sync-knowledge)**：审查阶段内嵌步骤，非独立委派任务。根据 `.trellis/knowledge-map.json` 的映射规则，自动将代码改动的经验沉淀到 errors.md、AGENTS.md、guidelines 等文档中

**加载技能**: 不需要（如果涉及架构理解可加 `flutter-dev`）

**前置阅读**:
- `.trellis/spec/guides/planning.md` — PRD 模板
- 被文档覆盖的模块的 `AGENTS.md`
...
task(
  category: "writing",
  run_in_background: true,
  prompt: "按照 .trellis/spec/guides/planning.md 的 PRD 模板，在 .trellis/tasks/ 下创建 Todo 插件任务的 prd.md。描述一个 Todo 插件的数据模型（title, done, dueDate, priority）和 UI 结构（列表页+编辑页）。",
);
```
...
**加载技能**: 见 `.trellis/skill-defaults.json` → `taskOverrides.static_analysis`
...
| 新建 drift Table | `unspecified-high` | `categoryDefaults.unspecified-high` + `unit_test` | `.trellis/spec/backend/database-guidelines.md`、`.trellis/spec/backend/plugin-data-flow.md` |
| 新增 DAO 方法 | `unspecified-high` | `categoryDefaults.unspecified-high` + `unit_test` | `.trellis/spec/backend/database-guidelines.md` |
| 新增页面/组件 | `visual-engineering` | `categoryDefaults.visual-engineering` | `.trellis/spec/frontend/component-guidelines.md`、`.trellis/spec/frontend/uiux-guidelines.md` |
...
| 写 PRD | `writing` | 无需加载 | `.trellis/spec/guides/planning.md` |
| 写模块文档 | `writing` | 无需加载 | 对应模块的代码和 AGENTS.md |
| 代码质量检查 | `quick` | `static_analysis` | `.trellis/workflow.md` |
...
| 知识同步 (sync-knowledge) | 不独立委派（审查阶段内嵌） | 无需加载 | `.trellis/knowledge-map.json`、`.trellis/workflow.md` |
...
| 任务类型 | 必读上下文 | 视情况读取 |
|----------|------------|------------|
| 通用任务 | `AGENTS.md`、涉模块 `AGENTS.md`、`.trellis/skill-defaults.json` | `.trellis/workflow.md` |
| 架构 / 插件 | `lib/src/AGENTS.md`、`lib/src/core/AGENTS.md`、`lib/src/core/plugin/AGENTS.md` | `.trellis/tasks/archive/plans/`、相关注册代码 |
| Drift / 数据层 | `.trellis/spec/backend/database-guidelines.md`、`test/AGENTS.md`、目标 DAO/Repository | `app_database.dart`、表定义、已有迁移 |
| UI / Widget | `.trellis/spec/frontend/component-guidelines.md`、目标页面/组件、`app_tokens.dart` | shared widgets、对应 widget test |
...
| 文档 / PRD | `.trellis/spec/guides/planning.md`、相关 AGENTS/guidelines、真实源码路径 | `.trellis/tasks/archive/plans/` 示例 |
| 验证 / CI | `.trellis/workflow.md`、`test/AGENTS.md` | 最近失败日志、已知 learnings |
...
agent-workflow.md 与 `.trellis/workflow.md` 的 Plan → Execute → Finish 流程配合：
...
|------|------|
| `AGENTS.md` | 项目概述、已知问题 |
| `.trellis/spec/backend/database-guidelines.md` | Drift/SQLite 建表和查询规范 |
| `.trellis/spec/frontend/component-guidelines.md` | Widget 编写规范、Token 使用 |
| `.trellis/workflow.md` | 完整开发流程规范 |
| `.trellis/spec/guides/planning.md` | PRD 模板和命名约定 |
| `lib/src/AGENTS.md` | 三层架构总览 |
| `test/AGENTS.md` | 测试隔离模式和 ProviderScope override |
| `lib/src/core/plugin/AGENTS.md` | 插件系统关键约定 |
| `.trellis/knowledge-map.json` | 知识同步映射规则 |
| `.trellis/skill-defaults.json` | 任务类型到 Skills 的默认映射（委派前必须读取） |
| `.trellis/workspace/alan/learnings/errors.md` | Bug 根因分析与预防清单 |

> 最后更新：2026-05-23（从历史工作流恢复并适配到 Trellis）| 本文件是代理任务委派的唯一权威参考。如有矛盾以本文件为准。
