...
接到任务后先判断模式。模式决定是否允许改文件、是否需要 PRD、验证做到哪一层。

| 模式 | 触发语句 | 允许动作 | 禁止动作 | 输出 |
|------|----------|----------|----------|------|
| Explore | “看看”“分析”“定位”“先不要改” | 只读搜索、读源码、读文档、跑只读命令 | 修改文件、格式化、自动修复、提交 | 事实、证据路径、风险、建议 |
| Plan | “给计划”“怎么做”“先交流” | 只读分析、列方案、拆步骤 | 未确认前修改代码 | 方案表、影响面、验收方式 |
| Build | “实现”“修复”“执行”“跑通” | 按计划修改代码和测试 | 超范围重构、无关格式化 | 改动说明、验证结果 |
| Verify | “验证”“跑测试”“检查” | 运行验证命令、汇总错误 | 擅自修代码（除非用户授权） | 命令结果、失败定位 |
| Review | “审查”“review” | 代码审查、风险排序、测试缺口 | 主动改代码 | findings 优先的审查报告 |
| Ship | “提交”“commit”“发 PR” | 原子提交、按要求生成提交信息 | 自动 push（除非用户要求） | 提交哈希、变更范围 |

### 执行前声明

进入 Build 或委派子任务前，先给出简短声明，避免 vibe coding 过程中边界漂移：

| 字段 | 说明 |
|------|------|
| 任务模式 | Explore / Plan / Build / Verify / Review / Ship |
| 任务分类 | 见 `.omo/guidelines/agent-workflow.md` 的 category |
| 加载技能 | 来自 `.omo/skill-defaults.json` 的合并结果 |
| 前置文档 | 本次已经或即将读取的 AGENTS / guidelines |
| 执行边界 | 允许改动与禁止改动 |
| 验证方式 | focused / standard / full 中的一种或组合 |

### 变更预算

实现前估计改动范围。实际发现需要明显扩大范围时必须暂停说明。

| 预算项 | 规则 |
|--------|------|
| 文件数量 | 预计 1-3 个文件为简单变更；超过 3 个或跨层修改需明确计划 |
| 架构边界 | 从插件层扩展到 shared/core，或引入新依赖时必须说明原因 |
| 数据边界 | 修改 Drift table、schemaVersion、迁移逻辑前必须先确认 |
| UI 边界 | UI 任务不得顺手改数据语义；数据任务不得顺手重做视觉 |
...
根据任务风险选择验证层级。默认使用 `standard`；如果存在已知历史失败，先跑 `focused` 获取本次改动证据，再说明 full 的阻塞原因。

| 层级 | 使用场景 | 命令 | 通过标准 |
|------|----------|------|----------|
| focused | 单文件/单模块改动、快速确认本次变更 | `flutter analyze` + 目标测试文件 | 本次相关 analyzer 和目标测试通过 |
| standard | 普通实现任务、提交前默认验证 | `flutter analyze` → `dart fix --dry-run` → 相关测试 → `dart fix --dry-run` | 0 error / 0 warning，相关测试通过，无可修复 lint |
| full | 跨层改动、提交前最终验证、发布前 | `flutter analyze` → `dart fix --dry-run` → `flutter test` → `dart fix --dry-run` | 全量通过；如有历史失败，必须列出失败来源和是否与本次改动相关 |

### 验证选择规则

| 改动类型 | 最低验证 | 备注 |
|----------|----------|------|
| 纯文档 | focused | 检查链接、路径、格式即可 |
| 数据层 / Repository | standard | 至少运行对应单元测试 |
| Widget / 页面 | standard | 至少运行对应 widget test；视觉改动建议截图检查 |
| 路由 / 插件注册 / 数据库 schema | full | 影响面大，必须全量验证或说明阻塞 |
| 只读分析 | 不需要 | 除非用户要求跑命令 |
